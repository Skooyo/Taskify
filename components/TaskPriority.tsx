...
const TaskPriority = ({ priority }: {priority: string}) => {
    ...
    return(
        <>
        <div className={`flex ${bgColor()} w-[95%] px-2 h-fit py-2 rounded-md drop-shadow-xl justify-center items-center text-white gap-2`}>
            {priority == "Urgent" && <TbAlertHexagon size={24}/>}
            {priority == "Important" && <LuAlertTriangle size={24}/>}
